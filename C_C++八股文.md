...
- malloc 和 free 是**标准库函数**，支持[覆盖](https://blog.csdn.net/cpongo3/article/details/93996121)；new 和 delete 是**运算符**，支持重载(不能重载new和delete本身，能重载operator new和operator delete)
...
- 注意对指针的操作，`typedef char * p_char` 和 `#define p_char char *` 区别巨大(https://zhuanlan.zhihu.com/p/358653481)
  ...
  {
      int a = 10, b = 20, c = 30;
      const PINT p = &a;	// const修饰的是p，p是不能被赋值的, const (int*) p, 因为typedef了，int*是一个整体，*只作用于int，const修饰的就是p
      *p = 100;			// 但是 *p 是可以变的
      // p = &c;  		// error
      cout << p << ' ' << *p << endl;
...
- sizeof 是**运算符**，并不是函数，结果在 **编译时得到** 而非运行中获得(new 和delete也是运算符，但是是运行时确定，因为分配的是动态内存)；strlen是字符处理的**库函数**
...
C++ 支持重载，C 语言不允许
C++ 相对与 C 增加了一些关键字，如：bool、using、dynamic_cast、namespace等等
C++ 新增了引用，引用型变量是其他变量的一个别名，我们可以认为他们只是名字不相同，其他都是相同的
C++	面向对象，C面向过程
~~~
